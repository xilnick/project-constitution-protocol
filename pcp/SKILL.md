...
1. **Firewall Architecture Intent from Implementation (bidirectional):** Keep intent and implementation on opposite sides of the wall. (a) Do not pollute source code files with monolithic descriptive comments, long design background text, or volatile relative documentation file paths. (b) Do not pollute PCP entries with implementation: no code snippets, type/interface definitions, API endpoint or route lists, database schema/table/field dumps, configuration values, CSS selectors/class names, or file-by-file module maps. PCP records *motivation, decisions, requirements, and concepts* — the high-level WHY and WHAT for constructing the system — never the HOW. The `@pcp:<type>-<xxxx>` source anchor is the only link to code; point to implementation, never paste it.
...
**Entries are high-level records, not a code mirror.** Each entry captures *motivation, the decision/requirement/concept, and its rationale* — the WHY and the WHAT for constructing the system. It MUST NOT restate the HOW: no code snippets, type/interface definitions, API endpoint or route lists, database schema/table/field dumps, config values, CSS selectors/class names, or file-by-file module maps. Reference implementation only through the `@pcp:` source anchor — point to code, never paste it. **Litmus test:** if a line would have to change on a pure refactor that leaves the system's intent intact, it belongs in code, not PCP.
...
- **Record Intent, Not Implementation:** PCP entries are high-level system records (motivation, decisions, requirements, concepts). Never paste code, type/interface definitions, API endpoints/routes, schema/table/field listings, config values, or file-by-file maps into an entry — link to the code with a `@pcp:` anchor instead. When minting or editing, if a passage describes HOW something is built rather than WHAT is required and WHY, cut it. Keep each entry to its decision/requirement plus rationale (and, for `r`, acceptance criteria at the behavioral level). An entry that mirrors code is a defect — the code is the implementation's source of truth, PCP is the intent's.
