...
A constitution belongs to one codebase: enable this plugin per project, not globally. When the skill
is activated in a workspace:
...
0. **Resolve the CLI once.** The script ships beside this file, so its location depends on how the
   ...
   ```bash
   PCP="${CLAUDE_PLUGIN_ROOT:-}/skills/pcp/scripts/pcp.js"      # the plugin, enabled per project
   [ -f "$PCP" ] || PCP="pcp/scripts/pcp.js"                        # vendored into the project
   [ -f "$PCP" ] || PCP="$HOME/.claude/skills/pcp/scripts/pcp.js"   # legacy user-skill install
   ```
...
**Litmus test for any line in an entry:** if it would have to change on a pure refactor that leaves the system's intent intact, it belongs in code, not PCP.
...
- **Record Intent, Not Implementation:** an entry that mirrors code is a defect. Apply the litmus test in §3 when minting or editing: the code is the implementation's source of truth, PCP is the intent's.
