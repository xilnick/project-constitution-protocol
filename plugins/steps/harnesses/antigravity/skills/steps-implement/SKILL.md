...
One `steps-implementer` executes items in order via `procedures/step-planning.md`. Without a prompt,
it resumes the next pending item from `PLAN.md` on disk. In parallel waves, implementers edit
disjoint `owns` zones or worktrees. With no plan, the verification command is the item list — make
the change, run it, report. The implementer never reviews itself or commits.
