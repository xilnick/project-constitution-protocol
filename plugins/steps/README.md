...
one clean-context reviewer checks it, an implementer executes and self-checks, one clean-context
reviewer checks the implementation, and the orchestrator runs every gate itself before calling
anything done.
...
A phase composes three stages, and only one of them always runs:
...
`steps-planner`, `steps-plan-reviewer`, `steps-implementer`, `steps-impl-reviewer`,
`steps-architect-pro`, `repo-scout`. Each is a standing brief for its role and can be dispatched
directly, not only through a stage.
...
The six roles map onto two model tiers — cheap fast models for volume work, heavy models for
planning and critique. The routing (role→tier, the complexity gate, per-harness model
...
The full wave is heavy machinery. A reviewed phase costs five or more agent dispatches, a directory
